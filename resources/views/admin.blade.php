<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight ml-4">
            {{ __('Admin Dashboard') }}
        </h2>
        <div class="flex space-x-8 mt-4 ml-4">
            <x-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')">
                {{ __('Home Page') }}
            </x-nav-link>
            <x-nav-link href="{{ route('spaces.index') }}" :active="request()->routeIs('spaces.index')">
                {{ __('My space') }}
            </x-nav-link>
            <x-nav-link href="{{ route('spaces.bookings', ['space' => 1]) }}" :active="request()->routeIs('spaces.bookings')">
                {{ __('Bookings') }}
            </x-nav-link>
            <x-nav-link href="{{ route('spaces.create') }}" :active="request()->routeIs('spaces.create')">
                {{ __('Create Space') }}
            </x-nav-link>
        </div>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <p>Welcome to your admin dashboard!</p>
                </div>
            </div>
        </div>
    </div>
</x-app-layout>
